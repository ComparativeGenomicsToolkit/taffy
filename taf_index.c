...
static void usage(void) {
    ...
    fprintf(stderr, "-T --threads N : Worker thread count.  For --universal this controls BOTH the phase-2 OpenMP worker pool (each in-flight worker holds ONE genome's runs[] in RAM -- tens of GB for vertebrate-scale giants, so memory peak ~ N * 40 GB; rule of thumb N <= total_RAM_GB/80) AND bgzf decompression (capped internally at TAFFY_MAX_BGZF_THREADS=8 -- past that the htslib pool's coordination overhead overtakes any decompress win).  Default 1.\n");
...
int taf_index_main(int argc, char *argv[]) {
    ...
    int n_threads = 1;        // -T value; drives both bgzf (auto-capped at 8)
                              // and phase-2 OMP workers
    ...
    // bgzf threads are capped at TAFFY_MAX_BGZF_THREADS=8 inside the setter
    // -- past that the htslib mt-pool overhead dominates the decompress
    // win.  Phase 2 still uses the full -T value below.
    ...
    if (universal) {
        ...
        st_logInfo("Phase 2 worker threads : %d (memory ~ N * 40 GB per giant ancestor)\n", n_threads);
        rv = tui_create(li, tui_fn, tmp_dir, genome_name_map, n_threads);
        free(tui_fn);
